...
export type MoveArgsByType<TArg> = {
  PlaceNew: { destination: TArg };
  MoveEntity: { entity: TArg; destination: TArg };
  RemoveEntity: { entity: TArg };
  TakeFrom: { source: TArg; destination: TArg };
  SetState: { entity: TArg; state: TArg };
  ForEach: { targets: TArg };
  Shuffle: { target: TArg };
  SetActivePlayers: {};
  EndTurn: {};
  PassTurn: {};
...
type PartialArgsByType<M extends MoveType, TArg> =
  M extends MoveType ? Partial<MoveArgsByType<TArg>[M]> : never;
...
export type SetStateDoPayload = MovePayload & {
  arguments: {
    entity: EngineEntity;
    state: { property: string; value: JsonValue };
  };
};
...
export type PreparedMoveArgumentsByType<M extends MoveType = MoveType> =
  PartialArgsByType<M, PreparedMoveArgument>;
...
export type PreparedMovePayload<
  TArguments extends PreparedMoveArgumentsMap = PreparedMoveArgumentsMap
> = {
  arguments?: Partial<TArguments>;
};
...
export type SimulatedMoveArgumentsByType<M extends MoveType = MoveType> =
  PartialArgsByType<M, SimulatedMoveArgument>;
