 ...
 */
export type ValueRef<T = unknown> = CtxPathRef<T> | ContextPathRef<T> | GamePathRef<T> | ExpressionRef<T> | RelativeCoordinatesRef<T> | CoordinatesRef<T> | RelativePathRef<T> | ParentRef<T> | MapRef<T> | MapMaxRef<T> | PickRef<T> | CountRef<T>;
/**
...
export type ConditionTyped = ConditionHasLine | ConditionIsFull | ConditionNoPossibleMoves | ConditionPosition | ConditionContainsSame | ConditionIs | ConditionContains | ConditionNot | ConditionOr | ConditionSome | ConditionEvery | ConditionInLine | ConditionEvaluate | ConditionWould;
...
export interface InitialPlacement {
    ...
    entity: EntityMatcher<EntityAttributes<Entity>> & {
        state?: EntityState;
    };
    destination: {
        index?: number;
